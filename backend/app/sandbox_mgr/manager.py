    ...
    def acquire(self, session_id: str, gpu: bool = False) -> str:
        ...
        try:
            container = self._client.containers.run(
                self.image, name=name, detach=True,
                device_requests=device_requests,
                publish_all_ports=True,  # 接管 §2.3:容器内服务对外可访问
                tty=True, stdin_open=True,
                # 共享 pip 缓存卷:每会话独立容器,但 pip 下载的包复用(避免每次重装 aerosandbox 40s+)
            ...
            if "Conflict" in str(e) or "already in use" in str(e):
                ...
                container = self._client.containers.run(
                    self.image, name=name, detach=True,
                    device_requests=device_requests,
                    publish_all_ports=True, tty=True, stdin_open=True,
                    volumes={"agent-hub-pip-cache": {"bind": "/root/.cache/pip", "mode": "rw"}},
                )
            else:
                raise
